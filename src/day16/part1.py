...
def solve(rounds_left: int, current: Valve, unopened: list[Valve], distances: dict[str, dict[str, int]]) -> int:
    if rounds_left <= 0:
        return 0
    ...
    for i in range(len(unopened)):
        ...
        score = max(score, valve_to_open.rate * new_rounds_left + solve(new_rounds_left, valve_to_open, new_unopened, distances))
...
def main() -> None:
    ...
    print(solve(30, valves["AA"], [valve for valve in valves.values() if valve.id != "AA" and valve.rate > 0], distances))
